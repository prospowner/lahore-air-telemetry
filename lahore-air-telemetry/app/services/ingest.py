...
def fetch_and_store_owm_data():
    """Fetches air pollution data from OpenWeatherMap for all zones."""
    ...
    for zone in zones:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # OpenWeatherMap provides AQI on a 1-5 scale, we can scale or store directly,
                # but let's capture PM2.5 and PM10 components precisely
                ...
                db.add(log)
                print(f"[OWM] Logged telemetry for {zone.name} (AQI: {aqi})")
            else:
                print(
...
def fetch_and_store_waqi_data():
    """Fetches air quality feed from WAQI for all zones."""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get("status") == "ok":
                    ...
                    db.add(log)
                    print(f"[WAQI] Logged telemetry for {zone.name} (AQI: {aqi})")
                else:
                    print(
